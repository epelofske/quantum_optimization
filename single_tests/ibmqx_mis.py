...
def combined_test():
        ...
        ising2 = []
        x = []
        print('ibmqx, mis, powell, qasm simulator, no noise, gnp graph')
        print(maximum_independent_set(G))
        for a in range(1, 50):
             x.append(a)
#             optimizer = POWELL()
             optimizer = POWELL()
             result_out = solve_ibmqx_ising_qubo(G, max_independent_set_qubo_matrix_ibmqx, optimizer, a)
             ...
             ref = is_independent_set(result2, G)
             if ref == True:
